...
Shader "WahnStudio/HeightShader" {		  
    Properties{
		// Degree of curvature
		_clampColorCurve("clampColorCurve", Float) = -1
...
#pragma target 3.0
		...
		uniform float _clampColorCurve;
	...
	sampler2D _LastLevelTexture;
        ...
        void surf(Input IN, inout SurfaceOutputStandard o) {
			...
			float3 vc=IN.worldPos;
			//IN.worldPos = sqrt(-_Curvature - IN.worldPos.x * (IN.worldPos.z*IN.worldPos.z)* sqrt(IN.worldPos.x));
			float3 vu = IN.worldPos;


			if (_clampColorCurve > -1)
			{
				vu = IN.worldPos;
			}
			else
